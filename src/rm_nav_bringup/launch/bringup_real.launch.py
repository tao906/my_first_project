...
def generate_launch_description():
    ...
    declare_serial_port_cmd = DeclareLaunchArgument(
        'serial_port',
        default_value='/dev/ttyUSB0',  # 默认端口，根据实际情况修改
        description='Serial port for STM32 chassis connection')
    
    declare_serial_baud_cmd = DeclareLaunchArgument(
        'serial_baud',
        default_value='115200',        # 默认波特率
        description='Baudrate for STM32 chassis connection')
    ...
    bringup_pointcloud_to_laserscan_node = Node(
        ...
        remappings=[('cloud_in',  ['/processed_pointcloud']),
                    ('scan',  ['/scan'])],
        parameters=[{
    ...
    start_serial_bridge_node = Node(
        package='serial_bridge',       # 你的功能包名
        executable='serial_bridge',    # setup.py 中设置的入口点名称
        name='stm32_driver',           # 节点运行时的名称
        output='screen',
        parameters=[{
        ...
        # remappings=[
        #     # 关键：将节点内部的 /cmd_vel 重映射到 /cmd_vel_chassis
        #     # 这样节点收到的就是经过坐标转换后的最终底盘速度
    ...
    ld.add_action(start_filter_node)
