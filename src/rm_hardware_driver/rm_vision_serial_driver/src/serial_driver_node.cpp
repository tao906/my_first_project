...
// #include "rm_vision_serial_driver/serial_driver_node.hpp"
...
// // project
// #include "rm_vision_serial_driver/uart_transporter.hpp"
...
//   hp_publisher_ = this->create_publisher<rm_decision_interfaces::msg::RobotStatus>("/robot_status", 10);
//   // Task thread
...
//       auto hp_msg = rm_decision_interfaces::msg::RobotStatus();
//       RCLCPP_INFO(get_logger(), "当前mode=%f", receive_data.yaw);
//       hp_msg.current_hp  = receive_data.hp*2;
//       hp_publisher_->publish(hp_msg);
//        RCLCPP_INFO(get_logger(), "当前hp=%u", hp_msg.current_hp);
...
: Node("serial_driver", options) {
  FYT_REGISTER_LOGGER("serial_driver", "~/fyt2024-log", INFO);
  hp_publisher_ = this->create_publisher<rm_decision_interfaces::msg::RobotStatus>("/robot_status", 10);
  game_status_publisher_   = this->create_publisher<rm_decision_interfaces::msg::GameStatus>("/game_status", 10);
  // Task thread
  listen_thread_ = std::make_unique<std::thread>(&SerialDriverNode::listenLoop, this);
}
...
void SerialDriverNode::listenLoop() {
  ...
  while (rclcpp::ok()) {
    if (protocol_->receive(receive_data)) {
      ...
      serial_receive_data_pub_->publish(receive_data);
      auto hp_msg = rm_decision_interfaces::msg::RobotStatus();
      auto game_status_msg = rm_decision_interfaces::msg::GameStatus();
       RCLCPP_INFO(get_logger(), "hp=%u", receive_data.hp);
      hp_msg.current_hp  = receive_data.hp*2;
      
      
      if (hp_msg.current_hp >=310 &&hp_msg.current_hp<=400)
      {
          game_status_msg.game_progress = 1;
      }
      else if (hp_msg.current_hp > 10&&hp_msg.current_hp<=250)
      {
          game_status_msg.game_progress = 2;
      }
      else if (hp_msg.current_hp == 0)
      {
          game_status_msg.game_progress = 3;
      }
      
    
      hp_publisher_->publish(hp_msg);
      game_status_publisher_->publish(game_status_msg);
       RCLCPP_INFO(get_logger(), "当前game_status=%u", game_status_msg.game_progress);
