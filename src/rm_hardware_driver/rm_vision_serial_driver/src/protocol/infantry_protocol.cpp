...
#include "rm_vision_serial_driver/protocol/infantry_protocol.hpp"
...
void ProtocolInfantry::send(const rm_interfaces::msg::GimbalCmd &data) {
  ...
  packet.loadData<float>(static_cast<float>(data.pitch+1.0), 2);
  ...
  //   std::cout << "fire_advice (10进制): " << static_cast<float>(data.fire_advice) << std::endl;
  std::cout << "pitch (10进制): " << data.pitch << std::endl;
  // std::cout << "yaw (10进制): " << data.yaw << std::endl;
